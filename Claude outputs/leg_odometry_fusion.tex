...
($\dot R$). The implementation keeps only the first term, approximating the
body-frame derivative with a finite difference and applying the
\emph{current} tick's rotation to it:
\begin{equation}
\mathbf{v}_{\text{body}}^{\,W} \;\approx\; -R_k \cdot \frac{\mathbf{p}_{\text{foot}}^{\,B}(t_k) - \mathbf{p}_{\text{foot}}^{\,B}(t_{k-1})}{\Delta t}.
\label{eq:vel-approx}
\end{equation}
To see exactly what this drops, compare it against differencing the two
already-rotated positions instead, $[R_k\mathbf{p}_k - R_{k-1}\mathbf{p}_{k-1}]/\Delta t$.
Expanding that:
\begin{equation}
\frac{R_k\mathbf{p}_k - R_{k-1}\mathbf{p}_{k-1}}{\Delta t}
= R_k\frac{\mathbf{p}_k - \mathbf{p}_{k-1}}{\Delta t} + \frac{(R_k - R_{k-1})\,\mathbf{p}_{k-1}}{\Delta t},
\end{equation}
and the second term is exactly a finite-difference approximation of the
$\dot R\,\mathbf{p}_{\text{foot}}^{\,B}$ term missing from
Eq.~\eqref{eq:vel-approx}. So the estimator is implicitly assuming the
body's orientation does not change appreciably within one control tick
($\Delta t = 0.02\,\text{s}$) -- a small-rotation-per-timestep
approximation. It is reasonable for a walking gait at $50\,\text{Hz}$, but
it is a genuine, identifiable source of extra error during fast pitch/roll
motion (for instance, right at footfall), not merely floating-point noise.

Once $\mathbf{v}_{\text{rel}}^{\,W} = R_k(\mathbf{p}_k - \mathbf{p}_{k-1})/\Delta t$ is
...
\noindent \textbf{Step 3 -- velocity.} The body-frame finite difference is
unchanged from Step 1 (it doesn't involve the IMU either):
\begin{equation}
\frac{\mathbf{p}_{\text{foot}}^{\,B}(t_k) - \mathbf{p}_{\text{foot}}^{\,B}(t_{k-1})}{\Delta t}
= (0.297093,\ -0.000119,\ 0.002378)\ \text{m/s}.
\end{equation}
Rotating this by the current tick's full $R(\psi_k,\theta_k, \varphi_k)$ gives
$\mathbf{v}_{\text{rel}}^{\,W} = (0.277871,\ 0.104413,\ -0.012476)\ \text{m/s}$,
so the estimated body velocity is the sign-flipped result,
\begin{equation}
\mathbf{v}_{\text{body}}^{\,W} = (-0.277871,\ -0.104413,\ 0.012476)\ \text{m/s}.
\end{equation}
The dominant component is in $-x$ and $-y$ together, at a combined speed
of $\sqrt{v_x^2+v_y^2} \approx 0.297\,\text{m/s}$: since the hip/knee opened
up (extended the leg forward relative to the body) while the foot stayed
planted, the body must have moved forward over that foot, and with the
body's heading rotated about $20^\circ$ from where the estimator started,
that forward motion shows up split between the world's $x$ and $y$ axes
rather than along $x$ alone.
